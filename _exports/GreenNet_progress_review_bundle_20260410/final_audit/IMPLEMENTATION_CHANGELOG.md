...
- `final_audit/verification/final_evaluation_smoke_recheck/final_evaluation_report.md`
  - Rationale: recheck output proving the final evaluation regenerates successfully with the current machine state.
- `final_audit/verification/final_evaluation_smoke_recheck/final_evaluation_summary.csv`
  - Rationale: machine-readable recheck output from the regenerated final-evaluation command.
- `final_audit/verification/final_evaluation_smoke_recheck/final_evaluation_summary.json`
  - Rationale: machine-readable recheck output from the regenerated final-evaluation command.
...
- `results/20260409_164637__policy-heuristic__scenario-normal__seed-0__tag-verify_smoke_recheck/`
  - Rationale: real experiment smoke artifact from the clean recheck pass.
...
- `pyproject.toml`
  - Rationale: added tracked optional dependency groups for `test` and `train`, and pinned the supported Python range to the verified toolchain.
- `configs/README.md`
...
- `frontend/greennet-ui/src/pages/SimulatorPage.tsx`
  - Rationale: surfaces demo-data and generated-topology fallback states explicitly, and removes dead code that broke the TypeScript build.
...
- Restoring every historical run folder/checkpoint referenced by older matrix notes
  - Deferred because the final submission now cites the preserved official bundle and traceability artifacts instead of pretending that every historical raw artifact is required in-repo.
