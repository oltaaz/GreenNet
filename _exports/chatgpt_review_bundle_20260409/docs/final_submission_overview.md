...
7. Use `artifacts/final_submission/matrix_v6/manifest.json` and `traceability.csv` as the reviewer-facing map for the final claim.
...
The preserved report in `experiments/official_matrix_v6/final_evaluation/` is still the authoritative citation target; the `final_audit/verification/` bundle is supplemental verification material.
